...
Neste capítulo é apresentada a fundamentação teórica utilizada no desenvolvimento deste trabalho. Numa primeira etapa, são apresentados a terminologia e principais conceitos relacionados ao {\em Particle Swarm Optimization} (PSO) necessários à compreensão da abordagem proposta pelo presente trabalho. 

Posteriormente, são apresentados conceitos e definições das áreas de otimização multiobjetivo, haja visto que tais conceitos são importantes durante a utilização da dominância Pareto pelos componentes {\em gbest} e {\em pbest} do PSO. Também são relacionados os conceitos referentes à busca local, adaptada do método {\em Neutral Pareto Improvement} (NPI), para varredura nas vizinhanças do conjunto de soluções não-dominadas ao final de cada iteração com intuito de refinar as soluções encontras. 

Ao final do capítulo são relacionados os principais conceitos das áreas de mineração e classificação de dados utilizados durante a implementação do algoritmo proposto. Foram também destacados diversos
trabalhos relevantes, referentes ao tema abordado.
...
%Diversas tentativas têm sido utilizadas na aplicação de algoritmos evolucionários para a realização das tarefas de classificação de dados e pretende-se o PSO parece uma heurística particularmente interessante por apresentar como vantagens a utilização de um conceito simples, ser de fácil implementação, tipo de memória inerente, poucos de parâmetros de controle e eficiência computacional quando comparados a outros métodos encontrados na literatura \cite{Wang_2007}.
...
No contexto de PSOs baseados em permutação, \citeonline{Pan_2008} propõem uma nova abordagem de PSO e, basicamente, o processo de atualização da posição da partícula envolve os seguintes conceitos, tal como apresentado na formulação da Eq. \ref{eq:dpso_pos_permut}: 
...
Segundo \citeonline{Pereira_2014}, técnicas de nicho são uma funcionalidade importante, pois permitem ao algoritmo obter boas regras para todas as classes do problemas simultaneamente, em uma única execução. Além disso, o custo computacional total é reduzido em comparação com execuções múltiplas de uma versão do algoritmo que implemente uma população de soluções candidatas que descreve uma classe somente.

\section{Otimização Multiobjetivo}
\label{sec:multi_obj}

De acordo \citeonline{Cunha_2014} muitos problemas encontrados na natureza são modelados como sendo de múltiplos objetivos e muitos esforços têm sido empregados na construção de versões multiobjetivos de Algoritmos Evolucionários para resolução de tais problemas.

Em problemas multiobjetivo não existe apenas uma solução, mas sim um conjunto de soluções. Elas relacionam os ``{\em trade-off}'' entre os objetivos, e para tanto, uma noção de otimalidade é adotada para identificação destes conjuntos de soluções. 

Por isso, para problemas multiobjetivo é comum a utilização do conceito de otimalidade proposto por Francis Ysidro Edgeworth (em 1881). Este conceito foi difundido, posteriormente, por Vilfredo Pareto (em 1896), sendo assim denominado de {\em Edgeworth-Pareto ótimo} ou, simplesmente, {\em Pareto ótimo} \cite{Coello_2006}. É utilizado pelo PSO proposto na identifição das soluções consideradas ótimas para o problema de classificação do dados.

Normalmente, problemas multiobjetivo caracterizam-se pela maximização (ou minimização) de um vetor de objetivos, sendo descrito por um modelo matemático multiobjetivo conforme apresentado na formulação \ref{eq:multi_obj}:
...
Tal que $\Omega$ é o espaço de decisões. Sejam  $u = (u_1,u_2,...,u_m)$ e $v = (v_1,v_2,...,v_m)$ tal que $u,v \in \mathbb{R}^m$. Diz-se que $u$ domina $v$, ou $u \prec v$, se $\forall u_i \leq v_i$, para todo $i=1,2,...,m$, e $\exists\ u_i < v_i$, respeitando, se existir, as restrições do problema. 

Quando $u$ não domina e nem é dominado por $v$, diz-se que ambos $u$ e $v$ não possuem relação de dominância entre si ou são incomparáveis.
...
Assim, um {\em Conjunto Pareto-Ótimo} $\mathcal{P}$ é definido por:
...
A tarefa de encontrar a fronteira Pareto exata de um problema de otimização multiobjetivo pode ser difícil. Logo, são aceitáveis aproximações de uma fronteira Pareto encontradas dentro de um intervalo limitado de processamento computacional \cite{Coello_2006}. Tais soluções devem está o mais próximo da fronteira Pareto real com a maior diversidade possível de amostras. 

A otimização multiobjetivo é uma importante área de pesquisa para cientistas e engenheiros. Técnicas tradicionais de otimização, tais como métodos baseados em gradientes, são difíceis, senão impossíveis, de se utilizar em verdadeiros problemas de otimização multiobjetivo. Dessa forma, abordagens evolucionárias têm sido desenvolvidas para serem aplicadas em problemas de otimização multiobjetivo \cite{Hu_2003}. 
...
A relativa simplicidade do PSO em conjunto com suas técnicas de compartilhamento de informação tornaram-o um candidato natural para desenvolvimento de uma variante do PSO para aplicação em problemas de natureza multiobjetivo \cite{Mishra_2016}. 

Contudo, apesar do PSO se mostrar particularmente adequado à abordagem multiobjetivo, devido ao seu poder de convergência para problemas mono-objetivo \cite{Coello_2004}, parece que ainda não há um consenso acerca da melhor forma de representação dos componentes {\em gbest} e {\em pbest} no contexto multiobjetivo \cite{Reddy_2007}. 
...
Os algoritmos de {\em busca local Pareto} (PLS, {\em Pareto Local Search}) são técnicas de busca local nas vizinhanças de uma solução para resolução de problemas de otimização combinatória multiobjetivo. As PLSs aplicam uma estratégia de exploração iterativa em um conjunto de soluções objetivando obter novas soluções não-dominadas, a partir de pesquisas nas vizinhanças de cada solução do conjunto \cite{Drugan_2012}. 
...
Descrito no Algoritmo \ref{alg:BPI}, o {\em Best Pareto Improvement} (BPI) varre toda a vizinhança da solução $s$. O conjunto de soluções $A’$, retornado pela rotina, contém as soluções encontradas na vizinhança $\mathcal{N}(s)$ que dominam ou são incomparáveis com as soluções do conjunto $\{s\} \cup A$. 
...
Os avanços da tecnologia têm produzido uma abundância de dados e com isso, emprega-se normalmente métodos e técnicas capazes analisar e extrair conhecimento de bases de dados inutilizadas ou pouco aproveitadas. De acordo com \citeonline{Fayyad_1996}, por meio de KDD ({\em Knowledge Discovery in Database})\footnote{Tradução: Descoberta de conhecimento em banco de dados}, é possível extrair informações implícitas, não triviais, previamente desconhecidas e potencialmente úteis em um banco de dados, que muitas vezes são inutilizadas ou pouco aproveitadas dentro do ambiente de trabalho. 

A {\em mineração de dados} ({\em data mining}) é uma fase dentro do processo de KDD e consiste basicamente na aplicação de algoritmos que, sob aceitável eficiência computacional, possam contruibuir ao reconhecer padrões ou modelos em relação aos dados estudados \cite{Fayyad_1996_2}. Tem despertado a atenção de diversos estudiosos, sendo citada pelos mesmos como um elemento fundamental em pesquisas relacionadas a área estratégica industrial, já que permite extrair informações que alavanquem seus negócios bem como a descoberta de novas oportunidades de inovação \cite{Chen_1996}.

O termo mineração de dados foi cunhado em alusão ao processo de mineração. Por sua vez, o processo de extração de minerais preciosos pode ser compreendido da seguinte forma: explora-se uma base de dados (mina) usando algoritmos (ferramentas) para a obtenção de conhecimento (minerais preciosos) \cite{Castro_2016}. 

Diversas tentativas têm sido utilizadas na aplicação de algoritmos evolucionários para a realização das tarefas de classificação de dados. Neste contexo, a metaheurística PSO se apresenta como uma ferramenta promissora para aplicação em problemas de mineração de dados devido a sua facilidade de implementação e poucos parâmetros a serem ajustados \cite{Wang_2007} e considerado um método robusto à resolução de problemas não-lineares, não-integráveis, multimodais e de alta dimensionalidade \cite{Krohling_2004}.  

Além disso, o PSO é aplicado com sucesso na resolução de diversos problemas, tais como: reconhecimento de padrões, modelagem de sistemas biológicos, roteamento, processamento de sinal, dentre outros \cite{Eberhart_2001}. 


\subsection{Classificação de Dados}
\label{sec:classif_dados}

Classificar um objeto significa atribuir um rótulo, chamado {\em classe}, de acordo com a categoria à qual o objeto pertence. Para que isso seja possível, um algoritmo ou método é utilizado na construção de um modelo de classificação (também chamado de {\em classificador}). Há uma variedade de algoritmos destinados à tarefa de classificação de dados na literatura, tais como: a {\em Árvores de Decisão}, as {\em Rede Neurais Artificiais}, {\em Máquina de Vetores Suporte}, {\em Algoritmos Evolucionários}, etc, e como exemplos de aplicações de classificação de dados, pode-se citar a identificação de {\em spams}, atribuição de crédito e detecção de fraudes, além de outros \cite{Castro_2016}.

A {\em extração de regras} (do inglês {\em rule mining} ou {\em rule discovery}) é um método dentro da classificação de dados que tem como objetivo encontrar um conjunto de regras {\em SE-ENTÃO} para classificar uma base de dados de maneira mais ``natural''. Conforme \citeonline{Pereira_2012} a extração de regras é um tópico recorrente na mineração de dados, segundo \citeonline{Hassani_2013}, caracterizado por ser um problema {\em NP-Completo}.

Cabe observar que a obtenção de regras com alto grau de precisão na etapa de treinamento não é garantia de que o mesmo sucesso seja obtido na fase de testes. Isso ocorre pelo fato de que regras muito adaptadas a um determinado conjunto de dados tendem a ser ineficientes na classificação de novos dados ({\em overfitting})\footnote{Tradução: Sobregeneralização}, sendo incapazes de promover a generalização \cite{Pereira_2010}. Dessa forma, buscar um conjunto de regras que seja mais simples (com menor número de elementos) também é um objetivo explorado por alguns algoritmos que abordam o problema de classificação de dados (\citeonline{Pereira_2014}, \citeonline{Liu_2004}, \citeonline{Sousa_2004}, etc).

A Figura \ref{fig:modeloPSO} ilustra o processo de classificação de dados através de conjunto de regras {\em SE-ENTÃO}: 

\begin{enumerate}[label={(\roman*)}]
\item Coleta de dados;
\item São extraídas as regras de classificação durante a fase de treinamento;
\item Constroí-se o modelo de classificação;
...
Normalmente, os algoritmos para problemas de classificação de dados raramente utilizam a abordagem Pittsburgh. De acordo \citeonline{Cervantes_2005}, a abordagem Michigan apresenta certas vantagens em relação à abordagem Pittsburgh: (i) Capacidade de fornecer boas soluções com um número menor de avaliações da função-objetivo; (ii) Maior flexibilidade na representação das regras de classificação.

A classificação de dados por meio de extração de regras estudada neste trabalho é modelado como um problema de otimização biobjetivo tal que se busca maximizar a efetividade das regras extraídas ao mesmo tempo em que se tenta minimizar a complexidade das regras (tamanho), uma vez que regras menores são mais simples de serem entendidas e tendem a evitar o {\em overfitting} \cite{Pereira_2012}.
...
\begin{itemize}
\item \textbf{Verdadeiros positivos} (VP): total de registros (ou tuplas) recuperados do banco de dados utilizando a regra codificada pela partícula nas quais as respectivos registros coincidem com a classe predita pela partícula; 
...
A tarefa de classificação de dados pode ser realizada por meio de diversas abordagens. A seguir são apresentados trabalhos relevantes encontrados no contexto de classificação de dados que serviram de referência para desenvolvimento desse trabalho.

\citeonline{Sousa_2004} apresentam um estudo acerca de uma variante do PSO destinada à classificação de dados por meio da mineração de regras de classificação {\em SE-ENTÃO} em uma abordagem mono-objetivo. A aptidão das partículas é avaliada a partir do cálculo do produto da sensibilidade pela especificidade. É interessante notar que as partículas, posteriormente à avaliação de aptidão, passam for uma fase de encolhimento ({\em prunning}), com o objetivo eliminar os atributos que não contribuem para eficácia da regra. O trabalho investiga as seguintes variantes do PSO: {\em Discrete PSO} (DPSO), {\em Constricted PSO} (CPSO) e {\em Linear Decreasing Weight PSO} (LDWPSO).

\citeonline{Liu_2004} apresentam um classificador baseado no PSO para classificação de dados chamado de {\em Rule Discovery with Particle Swarm Optimization} (REPSO). Neste classificador, cada partícula representa uma única regra e, segundo os autores, tem a vantagem de poder ser aplicada tanto em dados categóricas como em dados contínuos. Durante os testes, foram utilizadas duas bases de dados do {\em UCI repository of Machine Learning}: a base de dados {\em Zoo}, no qual todos os atributos são categóricos e o conjunto de dados {\em Wine}, no qual todos os atributos, exceto o atributo de classificação, são contínuos. As regras descobertas pelo algoritmo foram avaliadas pelo somatório da acurácia obtida e pelo número de termos utilizado pela regra, sendo atribuídos ``pesos'' para cada um, na avaliação da aptidão ({\em fitness}) da solução.
...
\citeonline{Hassani_2013} apresentam um DPSO para mineração de regras de classificação que utiliza uma separação dos dados de treinamento em blocos onde cada um é atribuído a uma {\em thread}. Ao final, as regras obtidas em cada {\em thread} são integradas em uma base de regras para construção de um modelo de classificação.

\citeonline{Mishra_2016} apresenta um classificador para mineração de regras de associação com abordagem multiobjetivo para maximização do {\em suporte} ({\em cobertura}) e {\em confiança} da regra de associação ao mesmo tempo em que tentar minimizar a complexidade da mesma.

A maioria dos trabalhos relacionados acima propõem estruturas de dados baseados em uma representação binária das partículas para extração de regras de classificação ou associação, similares ao DPSO proposto \citeonline{Kennedy_1997}. 

Em problemas de classificação de dados que levam em consideração a complexidade das regras são utilizados mecanismos para ignorar certos termos da regras {\em SE-ENTÃO} codificadas nas partículas. Geralmente, estas codificações envolvem a utilização de um vetor com todos os atributos da base de dados e um {\em bit} que habilita ou não a utilização de determinado atributo na regra. Neste contexto, o seguinte trabalho se torna relevante ao utilizar uma nova abordagem baseada no trabalho de \citeonline{Pan_2008} (Ver seção \ref{sec:atual_part}).

\citeonline{Pan_2008} propõem um processo de atualização da posição da partícula do PSO alternativo. Neste trabalho, o PSO proposto é aplicado em ambientes combinatoriais na resolução de problemas de alocação de recursos e escalomanento ({\em flow-shop}). Segundo os autores, tal abordagem pode ser facilmente aplicada em problemas de otimização combinatorial com resultados que demonstram ser competitiva, podendo inclusive obter resultados superiores a outros métodos encontradas na literatura especializada.

Por fim, é importante também mencionar o trabalho de \citeonline{Pereira_2012} que serviu de embasamento na utilização da metodologia aplicada neste trabalho. Buscou-se inspiração na modelagem proposta pelo autor para o problema de classificação de dados. No seu trabalho é proposto uma variante multiobjetiva baseada em Programação Genética (PG) para extração de regras de classificação sendo realizadas comparações de desempenho com três algoritmos implementados pelo WEKA.

\section{Conclusão}

Foram apresentados neste capítulo os principais conceitos e definições relacionados ao PSO. Fez-se uma descrição de suas características principais, ressaltando suas características e aplicações. Em seguida, foram relacionadas variantes do PSO destinadas ao ambiente combinatorial e as estratégias de geração do enxame inicial e técnicas de nicho.

Conceitos relacionados a otimização multiobjetivo, busca local Pareto e mineração de dados (classificação de dados), bem como a apresentação de estudos relevantes que serviram de referência para este trabalhos. Nota-se que ainda são poucos os que utilizam o PSO para a classificação de dados por meio da extração de regras. Não foi encontrado nenhum trabalho de classificação de dados baseado em PSO utilizando a formulação proposta por \citeonline{Pan_2008}, o que torna o presente trabalho inovador neste sentido.
...
Neste capítulo será detalhado o algoritmo biobjetivo, baseado na metaheurística PSO e denominado {\em multiobjective Discrete Particle Swarm Optimization} (mDPSO), que foi proposto neste trabalho para classificação de dados numéricos. 
...
\begin{itemize}
   ...
   \item Os parâmetros $I$ e $X$ representam respectivamente a regra codificada pela partícula e a classe $X$ (nicho) a que pertence a partícula;
   \item $f_1(I,X)$, descrita por Eq. \ref{eq:fitness1}, corresponde à função que determina a efetividade da regra. Esta efetividade é resultante do produto da sensibilidade pela especificidade; 
...
O objetivo do mDPSO é extrair um conjunto de regras durante as fases de treinamento para avaliar seu desempenho em novas amostras, desconhecidas durante a tarefa de treinamento.

A abordagem Michigan foi utilizada para representação das partícula do mDPSO. Dessa maneira, cada partícula codifica somente uma regra do problema o que significa dizer que cada partícula provê uma solução possível para determinada classe e não uma solução geral para todo o problema. 
...
\begin{algorithmic}[1]
\INPUT{$p.pbest$ - Soluções não-dominadas encontradas pela partícula $p$; $C_k.gbest$ - Soluções não-dominadas do nicho $C_k$}.
...
A função {\em merge}, tal como descrita na seção \ref{sec:busca_local_pareto}, agrupa os dois conjuntos Pareto em um novo conjunto Pareto, removendo as soluções dominadas do conjunto resultante.
...
Um ponto importante no mDPSO refere-se aos componentes {\em gbest} e {\em pbest}. Na versão multiobjetivo do PSO, ambos {\em gbest} e {\em pbest} passam a atuar como repositório de soluções não-dominadas, ao contrário da versão mono-objetivo que se referem somente à melhor posição obtida pela partícula ({\em pbest}) e a melhor posição obtida pelo enxame ({\em gbest}). Para o problema de classificação de dados investigado neste trabalho, cada partícula possui seu próprio repositório de soluções não-dominadas ({\em pbest}) e cada nicho um {\em gbest} específico ($C_k.gbest$).

O PSO, geralmente, implementa uma partícula líder para atualização das posições das partículas do enxame, ou seja, uma solução que contribui no direcionamento do enxame pelo espaço de busca, como ocorre na versão original do PSO. Nas versões mono-objetivo a partícula líder é o próprio {\em gbest}, sendo necessária a proposição de um mecanismo de identificação da partícula líder na abordagem multiobjetivo, visto que, neste contexto, o {\em gbest} é representado por um repositório (conjunto) de soluções não-dominadas. 

O mDPSO também implementa o {\em gbest} através de um repositório de soluções não-dominadas para cada nicho (classe) do problema. Além disso, o mDPSO também representa o {\em pbest} de cada partícula como um repositório de soluções não-dominadas. 

Dessa forma, o processo de identificação da partícula líder ({\em gbest} ou {\em pbest}), necessária à recombinação pelo mDPSO durante a operação de atualização da posição de cada partícula, ocorre da seguinte maneira: (i) Determina-se a distância euclidiana no plano dos objetivos da partícula corrente em relação às soluções no repositório ({\em gbest} ou {\em pbest}); e (ii) a partícula mais próxima da partícula corrente em cada repositório é selecionada para atualização da sua posição durante a recombinação. 

A escolha da partícula não-dominada  mais próxima da partícula corrente para operação de atualização da posição é exemplificada pela Figura \ref{fig:sel_lider}. A partícula escolhida (partícula líder) é realçada na cor vermelha e a partícula corrente na cor azul.
...
Tal abordagem é implementada no algoritmo de recrutamento de abelhas - {\em OptBees} \cite{Renato_2012}. O {\em OptBees} é um algoritmo inspirado no comportamento das abelhas durante o forrageamento e em mecanismos envolvidos no processo de alocação de tarefas da vida em sociedade do enxame para resolução de problemas de otimização em espaços contínuos. Também é importante frisar que tal abordagem pode ser aplicada tanto em problemas de maximização quanto de minimização.
...
A fase de construção (linha 7 do Algoritmo \ref{alg:mdpso_ger_inicial}) da regra de cada partícula se resume a definir os $\eta$ termos que entrarão na composição da regra associada à partícula. $\eta$ é um número aleatório inteiro no intervalo $[1, \  \ceil[\big]{\log{(rand_{[1, N]})}}]$ sendo $N$ o total de atributos da base de dados. Esta metodologia objetiva simplicar as regras introduzidas na codificação da partícula.
...
Muitos autores citam a importância do operador de turbulência em razão de promover a diversidade do enxame no PSO \cite{Parsopoulos_2008}, visto que a diversidade do enxame é importante para melhorar a capacidade exploratória do algoritmo PSO \cite{Reyes_2006}. Como afirmam \citeonline{Santana_2009}, o PSO mono-objetivo apresenta rápida convergência quando comparado a outros métodos, contudo, tal vantagem pode ser também prejudicial no contexto de otimização multiobjetivo, já que o algoritmo tende a convergir prematuramente à um ótimo-local. %A convergência prematura é causada pela rápida perda de diversidade do enxame no PSO. 

Para tentar atenuar tal efeito, o mDPSO utiliza um esquema adaptada do trabalho de  \citeonline{Reyes_2005}. Nesta proposta, cada nicho é subdividido em três subpopulações $C_1, C_2$ e $C_3$ de mesmo tamanho, de modo que cada subpopulação $C_i \ (i=1,2,3)$ pode, ou não, ser perturbada, via operador de mutação. Para isso utiliza a seguinte estratégia: (i) A subpopulação $C_1$ não sofre nenhuma operação de mutação; (ii) a mutação uniforme é aplicado nas partículas presentes em $C_2$; e (iii) a mutação não-uniforme é aplicada nas partículas da subpopulação $C_3$. Como alternativa à mutação não-uniforme é utilizado a mutação gaussiana \cite{Higashi_2003}. Na seção \ref{sec:mdpso_mutacao} são apresentadas as mutações implementadas pelo mDPSO.

\section{Atualização da Posição da Partícula}
\label{sec:mdpso_atual_pos_part}

A rotina de atualização da posição da partícula desenvolvida pelo mDPSO é descrita pelo Algoritmo \ref{alg:mdpso_atualiza_part}. A rotina utiliza operadores de mutação e recombinação específicos ao problema de classificação de dados, sendo uma adaptação da formulação proposta no trabalho de \citeonline{Pan_2008}, descrito na seção \ref{sec:atual_part}. 
...
Ao analisar o Algoritmo \ref{alg:mdpso_atualiza_part} observa-se um paralelo entre a Eq. \ref{eq:pso_vel}, equação de atualização da partícula implementada pelo PSO original, e a Eq. \ref{eq:paralelo_pso_vel} que corresponde à formulação proposta por \citeonline{Pan_2008}:
...
Existem discussões acerca do mecanismo de da atualização da posição da partícula no algoritmo PSO: (i) Para \citeonline{Reyes_2006}, o ajuste da velocidade da partícula atua como um operador de ``mutação direcional''; (ii) Para \citeonline{Coello_2004}, o ajuste da posição da partícula é análogo à recombinação ({\em crossover}) dos Algoritmos Genéticos. Diante dessas discussões, o mecanismo proposto por \citeonline{Pan_2008} mostrou se adequado ao processo de atualização da posição da partícula do PSO em problemas combinatoriais, visto que ele reune características de ambas abordagens e este foi o mecanismo implementado pelo mDPSO.

\subsection{Operação de Mutação de Partícula}
\label{sec:mdpso_mutacao}

De acordo \citeonline{Cunha_2014}, para cada problema podem existir várias representações computacionais às suas resoluções, sendo normalmente necessário efetuar uma análise experimental para encontrar perturbações (mutações) com valores aceitáveis. 

Deste modo, no decorrer de desenvolvimento deste trabalho, por meio de experimentos em laboratório, foram propostos três mecanismos de mutação nas regras das partículas objetivando garantir (ou mesmo introduzir) diversidade no enxame de partículo do mDPSO. Os mecanismos de mutação implementados pelo mDPSO são detalhados no Algoritmo \ref{alg:mut}.
...
É importante destacar que o algoritmo não implementa nenhum mecanismo de mutação que altere o rótulo (nicho) das partículas. Os três tipos de mutação nas regras das partículas implementados pelo mDPSO são detalhados nas seções \ref{sec:mdpso_mut1}, \ref{sec:mdpso_mut2} e \ref{sec:mdpso_mut3}.

\subsubsection{Adicão de Novo Termo}
\label{sec:mdpso_mut1} 

%Durante a recombinação da partícula, o segundo objetivo $f_2(I,X)$ (complexidadade da regra) ocasionava a perda de diversidade do enxame no mDPSO, ao privilegiar regras com um número menor de termos. Dessa forma, para contornar tal situação, foi proposta a seguinte mutação. 

Durante a recombinação da partícula, foi percebido que as partículas teriam, no máximo, regras com ($\ceil{log(N)} + 1$) termos, onde $N$ é o número total de atributos (Ver Algoritmo \ref{alg:mdpso_ger_inicial}). Diante deste fato, é necessário que o mDPSO implemente um mecanismo que aumente o número de termos de uma regra, objetivando construir regras com maior complexidade (tamanho) e elevada efetividade.  A mutação do tipo ``adição de novo termo'' é exemplificada na Figura \ref{fig:mut_1} com adição do termo ($Atributo5 < Valor5$) à regra. 
...
O valor da perturbação, a ser adicionado ou subtraído do elemento ``valor numérico'' do termo da regra, pode ser obtido de duas formas distintas \cite{Jancauskas_2014}: (i) Valor aleatório gerado segundo uma distribuição uniforme (linha 12 do Algoritmo \ref{alg:mut});  (ii) Valor aleatório gerado segundo uma distribuição normal (gaussiana) (linha 9 do Algoritmo \ref{alg:mut}).
...
O funcionamento do operador é ilustrado na Figura \ref{fig:mut_3}: ``NovoValor7'' refere-se ao valor resultante da adição/substração de um valor numério ao elemento ``valor7'' do termo da regra.

%Mecanismos de busca local são conhecidos pela sua eficiência, especialmente em problemas combinatórios complexos, mostrarando-se vantajosos em situações onde soluções superiores são encontradas próximas às soluções originais. 
...
Mecanismos de busca local são importantes por possibilitar refinamentos das soluções obtidas por um algoritmo de otimização combinatória. O Algoritmo \ref{alg:BLP} detalha o mecanismo de busca local Pareto, baseado no método {\em Neutral Improvement Pareto} (Ver Algoritmo \ref{alg:NPI}). Observou-se que o NPI foi a abordagem que apresentou os melhores resultados quando comparado com BFI e FPI, sendo por isso implementado no mDPSO. 

A rotina de busca local Pareto proposta utiliza um mecanismo de geração de soluções vizinhas (linha 4 do Algoritmo) baseado na mutação gaussiana \cite{Higashi_2003}. Esta mutação pode ser eficiente em situações específicas e apresentou bons resultados para os problemas de classificação de dados analisados neste trabalho. O método proposto é executada ao final de cada iteração do mDPSO com objetivo de introduzir novas soluções não-dominadas no repositório $C_k.gbest$.  
...
Este capítulo descreve o algoritmo biobjetivo, denominado mDPSO, baseado no algoritmo PSO para classificação de dados por meio de extração de regras. O algoritmo proposto implementa várias funcionalidades, tais como, por exemplo, técnicas de nicho, rotina para geração da população inicial, mecanismos de recombinação e mutação bem como um mecanismo de busca local Pareto. As partículas foram modeladas de maneira a codificar uma cláusula WHERE da linguagem SQL utilizados por vários sistemas de gerenciamento de bancos de dados. Dois objetivos foram considerados (efetividade e complexidade) a fim de obter boas regras de classificação de evitar overfitting. Outra vantagem desta abordagem é a busca por regras menores, mais fáceis de serem entendidas e interpretadas. 

%Foi possível perceber certas vantagens e desvantagens em relação ao PSO, tanto na maior flexibilidade de codificação da regra da partícula como no aumento da complexidade durante atualização da posição da partícula. Ainda, a necessidade de definição de novos parâmetros. 
...
O mDPSO, por sua vez, realiza o mesmo cálculo utilizando a melhor regra de cada classe (nicho), baseada no valor de $f_1(I,X)$ (Ver Eq. \ref{eq:fitness1}). Dessa maneira, em uma base de dados composta por três classes, o mDPSO determina três classificadores distintos, um para cada classe. Como afirma \citeonline{Handl_2007}, deseja-se que os resultados gerados pelos algoritmos multiobjetivo sejam tão bons (ou melhores) do que aqueles gerados por algoritmos mono-objetivo. No caso específico deste trabalho, espera-se que os resultados gerados pelo mDPSO sejam comparáveis com os resultados obtidos com os  algoritmos estudados, implementados pelo WEKA.
...
Sendo as bases de dados {\em DG1}, {\em DG2}, {\em Hepatitis} e {\em Wine} foram as mesmas analisadas no trabalho de \citeonline{Pereira_2014}.

Para avaliação do desempenho dos algoritmos utilizou-se a {\em validação cruzada k=10-fold} (10 partições) que consiste em dividir a base de dados em {\em k} subconjuntos, sendo {\em k-1} partições para treinamento e 01 (uma) para teste. Este processo de treinamento e teste é repetido com todos os {\em k} subconjuntos e a média dos resultados obtidos é utilizada como indicador da qualidade do modelo \cite{Castro_2016}.

A validação cruzada utilizada é estratificada e consiste em distribuir uniformemente as classes das amostras entre as partições. Por exemplo, dado um conjunto de dados dividido em duas classes e sabendo que a {\em classe A} possui 20\% dos objetos e a {\em classe B} possui 80\% das amostras. Ao se fazer a separação da base de treinamento e teste é garantido a manutenção da mesma proporção entre as {\em k} partições. Para os testes realizados, as mesmas partições são utilizadas em todos os algoritmos para tentar prover uma justa comparação entre todos os algoritmos analisados. 
...
Para o mDPSO, os parâmetros $\omega$, $c_1$ e $c_2$ foram definidas com os seguintes valores de 0.9, 0.8 e 0.8 respectivamente, sendo tais valores escolhidos por meio de experimentações. Foram utilizadas 90 partículas com critério de parada 30000 avaliações da função-objetivo em todas as bases de dados analisadas.

Os parâmetros utilizados pelos 03 (três) algoritmos implementados pela ferramenta WEKA, para cada base de dados, são listados na Tabela \ref{tab:config_weka}. A escolha destes parâmetros foi baseada no trabalho de \citeonline{Pereira_2014}, sendo utilizadas as melhores configurações obtidas por meio de experimentações durante o desenvolvimento deste trabalho:
...
Para todas as bases de dados analisadas, o p-valor obtido pelo teste ANOVA foi de aproximadamente 0 (zero). Deste modo, a Tabela \ref{tab:pvalor_diabetes} apresenta a relação entre os pares de algoritmos de acordo com o p-valor do teste {\em Tukey} obtido para a base {\em Diabetes}. Os resultados mostram que não houveram diferenças significativas pelo desempenho do produto da sensibilidade pela especificidade global para as 50 execuções entre os algoritmos J48 e SMO, o que sugere que ambos os algoritmos J48 e SMO apresentaram desempenho similar para esta base de dados. 
...
Na Tabela \ref{tab:pvalor_dga2} é apresentada a relação entre os pares de algoritmos de acordo com o p-valor obtido para a base {\em DGA 2}. Os resultados apresentados mostram que os valores gerados pelos algoritmos mDPSO é distintos dos demais algoritmos, apresentando desempenho superior. Por outro lado, os resultados mostrados na tabela sinalizam que não se pode afirmar que o algoritmo J48 possui desempenho superior ao do RBF. Diferentemente da base {\em DGA 1}, o SMO foi o que apresentou o pior desempenho, possivelmente devido ao grau de desabalanceamento da base {\em DGA 2} em comparação a base {\em DG1}.
...
Para a base {\em Hepatitis}, o gráfico da Figura \ref{fig:hepatitis} mostra o bom desempenho dos algoritmos RBF, mDPSO e SMO, sendo possível observar o desempenho superior do algoritmo RBF. Durante a avaliação entre os pares de algoritmos segundo o p-valor obtido para esta base, conforme resultados apresentados na Tabela \ref{tab:pvalor_hepatitis}, mostra que não houveram diferenças estatísticas significativas entre o mDPSO e o SMO, sendo que o RBF apresentou o melhor desempenho. O J48 apresentou o pior desempenho, bem abaixo dos demais algoritmos: RBF, mDPSO e SMO.
...
Para o conjunto de bases analisadas neste trabalho, a base {\em Unbalanced} caracteriza-se como a mais desabalanceada. Novamente, tal como ocorre na base {\em DGA 2}, o mDPSO apresentou resultados superiores em relação aos demais algoritmos. Possivelmente, o desempenho ruim destes algoritmos deve-se ao fato de que eles não foram capazes de identificar as duas classes da base, sendo penalizados quando avaliado o produto da sensibilidade pela especificidade.
...
A Tabela \ref{tab:pvalor_unbalanced} apresenta a relação entre os pares de algoritmos de acordo com o p-valor obtido para a base {\em Unbalanced}. Os dados mostram que os resultados do mDPSO não possuem nenhuma similaridade com os dos outros três algoritmos. 
...
Por fim, a Figura \ref{fig:wine} mostra o gráfico de desempenho dos algoritmos para a base {\em Wine}, segundo os valores obtidos para a métrica sensibilidade $\times$ especificidade. A figura ilustra o bom desempenho dos algoritmos SMO e RBF frente aos outros dois (mDPSO e J48). Além disso, nota-se que os algoritmos SMO e RBF apresentam desempenho semelhante, não sendo possível afirmar qual é o melhor dos dois. Avaliando os pares de algoritmos segundo o p-valor computado para a base {\em Wine}, conforme descrito na Tabela \ref{tab:pvalor_wine}, verifica-se que não houveram diferenças estatísticas significativa entre os algoritmos SMO e o RBF. Novamente, o J48 apresentou o pior desempenho. Para esta base, o mDPSO aprensentou desempenho intermediário, com diferenças estatísticas de desempenho entre o mDPSO e os demais algoritmos implementados pelo WEKA.
...
Na Tabela \ref{tab:sen_esp_global} é apresentada a média (desvio padrão) das 50 execuções do produto da sensibilidade pela especificidade global dos 05 (cinco) algoritmos para as 07 (sete) bases de dados analisadas neste trabalho. Observa-se que apesar do desempenho ruim nas bases {\em Ionosphere} e {\em Wine}, o mDPSO conseguiu bons resultados nas demais bases de dados. Particularmente, para as bases de dados {\em Diabetes}, {\em DGA 1} e {\em Unbalanced}, em que apresentou os melhores indicadores de desempenho. Além disso, apesar do bom desempenho do algoritmo PG na base {\em DGA 2}, o desvio padrão calculado para este algoritmo é superior (aproximadamente 435\%) ao desvio computado para o mDPSO, o que mostra o bom desempenho deste algoritmo para esta base. Para facilitar a compreensão dos dados da tabela, negritou-se os melhores resultados obtidos para cada base de dados.
...
A Tabela \ref{tab:rank_efet} apresenta o {\em ranking} de desempenho dos 04 (quatro) algoritmos para cada base de dados, de acordo estratégia similar à utilizada no trabalho de \citeonline{Pereira_2012}. Ao analisar os resultados exibidos na Tabela \ref{tab:rank_efet}, observa-se o bom desempenho do algoritmo mDPSO para classificação de dados em diversas bases de dados. 

Em resumo, o mDPSO obteve o melhor desempenho em 04 (quatro) das 07 (sete) bases avaliadas ({\em Diabetes}, {\em DGA 1}, {\em DGA 2} e {\em Unbalanced}). Para a base de dados {\em DGA 1} o mDPSO obteve desempenho similar ao algoritmo SMO e superior aos algoritmos J48 e o RBF. Deve-se atentar para o desempenho ruim apresentado pelo algoritmo J48 que não conseguiu se sobressair em nenhuma das bases de dados analisadas, geralmente ocupando posições intermediárias. 
...
É importante também mencionar que apesar de constar desempenho do algoritmo PG para algumas bases de dados não foi possível analisá-lo para inclusão no {\em ranking} de desempenho. 

Portanto, de maneira geral, foi possível observar que o mDPSO apresentou resultados bastante promissores, mostrando-se competitivo quando comparado à outros métodos clássicos encontrados na literatura especializada. Sendo importante destacar o bom desempenho obtido do mDPSO em relação às bases de dados desbalanceadas.

\section{Conclusão}

Neste capítulo foram apresentados os resultados de desempenho obtidos entre o mDPSO e 03 (três) algoritmos implementados pelo WEKA para 07 (sete) bases de dados selecionadas. O mDPSO chamou a atenção pelo desempenho considerado satisfatório, sobressaindo-se principalmente nas bases de dados desbalanceadas.
...
A classificação de dados é um problema recorrente dentro da área de mineração de dados e o presente trabalho propôs uma abordagem multiobjetivo baseada em PSO para classificação de dados por meio de extração de regras. Foram implementados uma implementação de uma rotina de geração da população inicial, um mecanismo de busca local Pareto para acelerar a convergência do algoritmo e operadores de mutação e recombinação específicos ao problema. 

Denominado de mDPSO, o algoritmo foi comparada com três métodos implementados pela ferramenta de mineração de dados WEKA para análise em 07 (sete) bases de dados. A partir dos resultados obtidos, observa-se que o mDPSO se mostrou competitivo, com desempenho considerado promissores quando comparado aos algoritmos implementados pelo WEKA, como foi apresentado durante a análise de resultados experimentais.

No entanto, percebe-se que alguns pontos do mDPSO ainda requerem maior estudo, pois certas funcionalidades foram desenvolvidas por meio de testes e experimentações, como por exemplo, as taxas de probabilidades atribuidas a cada operador lógico utilizadas durante a geração da população inicial e na operação de mutação em um operador na regra da partícula. 

Outro aspecto em relação ao mDPSO refere-se a capacidade do algoritmo atualmente apenas manipular atributos númericos, sendo necessário um a realização de pré-processamento quando necessária a utilização de atributos textuais ou mesmo não-convencionais (geográficos).

Observa-se que o mDPSO obteve resultados considerados satisfatórios, sendo que chama a atenção  a capacidade do algoritmo em lidar com bases de dados desbalanceadas quando comparados aos outros métodos estudados neste trabalho, pois boa parte dos problemas encontrados hoje possuem algum grau de desbalanceamento o que torna o algorimo proposto relevante, não somente do ponto de vista científico, como também do ponto de vista prático.
...
\begin{itemize}
\item Aprimorar o mecanismo de busca local Pareto do mDPSO visando refinar a qualidade das soluções encontradas pelo algoritmo. A motivação para este fato se deve ao baixo desempenho apresentado pelo mDPSO durante a classificação de dados em algumas bases de dados em comparação aos demais algoritmos;
...
\item É esperado estender o mDPSO para classificação de dados híbridos (banco de dados geográficos), uma vez que são poucos os trabalhos referentes à classificação de dados não-convencionais encontrados na literatura e acredita-se que o algoritmo poderia incluir tal funcionaliade sem grandes alterações em sua implementação atual;
\item Outro aspecto importante seria estender o mDPSO para as três variantes propostas por \citeonline{Pereira_2014}. O mDPSO somente implementa o seguinte mecanismo a fim de classificar o conjunto de testes ({\em One rule matching}). Dessa maneira, um comparativo geral entre os três mecanismos ({\em One rule matching}, {\em Counting of matching rules} e {\em Weighted count of matching rules}) poderia fornecer um melhor comparativo, uma vez que a metodologia de análise de resultados neste trabalho é inspirada em seu trabalho;
\item Uma investigação de desempenho do mDPSO com outras bases de dados também seria relevante. Além disso, outros algoritmos reconhecidos na literatura poderiam ser utilizados para auxiliar em mais comparações e análises.
\end{itemize}
